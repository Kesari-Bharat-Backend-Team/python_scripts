...
class LineData:
	...
	def addColumns(self):
		...
		for i in range(start, end):
			...
			if i in [393, 395, 396, 397, 698, 699, 700, 702, 715, 722]: continue

			# if i in [393, 394, 395 ]: continue
			query = f"""
			...
			#added_road_name_flag -> nearby_schema 

			if False: #run 1 colland at a time 
				query = f"""
				...
				# add column in main line and edited line 
				query = f"""

				alter table {schema_name}
				add column if not exists gov_drrp_road_id character varying(1000),
				add column if not exists gov_road_category character varying(1000),
				add column if not exists gov_road_name character varying(1000);
	...
	def addRaodNamesfromNearByRoads(self):
		...
		for i in range(start, end):
			...
			# 	alter table if exists {near_by_roads_path}
			# 	add column if not exists added_road_name_flag integer default 0;
			...
			query = f"""
				...
				{near_by_roads_path}
				where near_by_gov_road_id is not null 
			...
			for data in records:
				...
				state_name = 'rajasthan' #temporary 
